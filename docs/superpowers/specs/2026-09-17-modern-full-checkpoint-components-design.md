...
O registro será orientado pela arquitetura detectada do Modelo A. Cada entrada declara os slots obrigatórios, as assinaturas aceitas, a família de VAE, as restrições de armazenamento e o nível de suporte.
...
| Anima | Qwen3 0.6B | Qwen Image VAE | suportado |
...
- `unknown`: permite o merge existente, mas bloqueia a nova composição modular até a arquitetura ser reconhecida.

## 6. Arquitetura interna

### 6.1 `component_registry.py`

Módulo puro e declarativo responsável por:

- identificadores de arquitetura;
- definição dos slots por arquitetura;
...
O registro não carregará modelos e não dependerá da interface.

### 6.2 `component_bundle.py`
...
Ao selecionar **Full Checkpoint**, a interface inspeciona o Modelo A e mostra uma seção de componentes apenas quando a arquitetura registrada exigir composição modular.

Cada linha de slot terá:
...
- Arquitetura desconhecida: mantém o merge existente, mas bloqueia a composição modular.
...
Flux 1, Flux 2 Klein 4B/9B, Wan 2.x, Qwen-Image/Edit, Anima, Krea2, Z-Image e Lumina Image 2 terão casos próprios. Chroma e Ernie-Image permanecerão experimentais até terem fixtures e validação real no runtime correspondente.
...
1. contratos e registro de arquiteturas;
